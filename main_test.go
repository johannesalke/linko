...
func Test_requestLogger(t *testing.T) {
	...
	if !reflect.DeepEqual(expectedLogString, logBuffer.String()) {
		t.Errorf("Log string does not have expected shape in test 1")
	}
	if !reflect.DeepEqual(rr.Code, expectedStatusCode) {
		t.Errorf("Status code is not as expected in test 1")
	}
	//t.Skip()
